    ...
    try {
      ...
      .then(() => {
        ...
        absences.forEach(v => {
          ...
          const f = Math.floor;
          if(f(from / 100) <= f(encoded / 100) && f(to / 100) >= f(encoded / 100)) {
            let periods = [];
            if(f(from / 100) === f(encoded / 100) && f(to / 100) === f(encoded / 100)) {
                for(var i = obj.fromPeriod; i <= obj.toPeriod; i++) periods.push({period:i});
            } else if(f(from / 100) === f(encoded / 100) || f(to / 100) === f(encoded / 100)) {
                if(f(from / 100) === f(encoded / 100)) {
                    for(var i = obj.fromPeriod; i <= 10; i++) periods.push({period:i});
                } else {
                    for(var i = 7; i <= obj.toPeriod; i++) periods.push({period:i});
                }
            } else {
                for(var i = 7; i <= 10; i++) periods.push({period:i});
            }
            promises2.push(Attendance.update({
              status: purpose,
              reason
            }, {
              where: {
