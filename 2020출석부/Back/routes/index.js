...
router.patch('/activity', jwtCheck, async (req, res, next) => {
  ...
  for (idx in data) {
    ...
    const date = new Date(_date.getFullYear(), _date.getMonth(), _date.getDate() + 1);
    if (date.getFullYear() === Number(year) && (date.getMonth() + 1) === Number(month) && date.getDate() === Number(day)) {
  ...
  xlsx.writeFile(newWb, path.join(__dirname, '../documents/afterSchoolSchedule.xlsx'));
  res.status(200).json({});
});
...
router.patch('/activity-each', jwtCheck, async (req, res, next) => {
  ...
  if (Number(date1[0]) !== Number(date2[0]) || Number(date1[1]) !== Number(date2[1]) || Number(date1[2]) !== Number(date2[2])) {
    for (idx in data) {
      ...
      if (year === Number(date1[0]) && month === Number(date1[1]) && day === Number(date1[2]) || year === Number(date2[0]) && month === Number(date2[1]) && day === Number(date2[2])) {
        if (year === Number(date1[0]) && month === Number(date1[1]) && day === Number(date1[2])) {
          workIdx1 = idx;
        } else {
          workIdx2 = idx;
        }
...
router.patch('/teachers', jwtCheck, (req, res) => {
  ...
  xlsx.writeFile(newWb, path.join(__dirname, '../documents/managerSchedule.xlsx'));
  res.status(200).json({});
});

router.patch('/teacher-each', (req, res) => {
  const { date1, date2, selected1, selected2, floor1, floor2 } = req.body;
  let teacherIdx1 = null, teacherIdx2 = null;
...
router.get('/create/students', async (req, res) => {
  ...
  data.forEach(async v => {
    const c = await Class.findOne({ where: { [Op.and]: [{ grade: v.grade }, { class: v.class }]} });
    await Student.create({ name: v.name, number: v.number, classId: c.id, clubId: Math.floor(Math.random()*10 + 1) });
  })
  res.json({});
})
