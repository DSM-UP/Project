function set_calendar(day_callback) {
    ...
    function make_days(days, Day) {//만든 day들을 담을 배열, 복사할 원본 day
        ...
        let calendar = CreateAMonthlyCalendar(today_date.getFullYear(), today_date.getMonth() + 1);
        for (var i = 0; i < 6; i++) {
            for (var j = 0; j < 5; j++) {
                if (i != 0 || j != 0) {
                    days[i][j] = Day.clone();
                ...
                //console.log(month);
                day_callback(days[i][j], new Date(today_date.getFullYear(), month, days[i][j].get_day_number()));
            }
        }
