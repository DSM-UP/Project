function set_calendar(day_callback) {
  function FindYoilOfOneMonthOneDay(year) {
    var yun = 0;
    var yoil = year - 1;
    for (var i = 1; i <= year; i++)
      if ((i % 4 == 0 && i % 100 != 0) || i % 400 == 0) yun++;
    yoil += yun;

    if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) yoil -= 1;
  ...
  function ThisMonthFirstDay(year, month) {
    ...
    var yun = 0;
    if ((year % 4 == 0 && year % 100 != 0) || year % 400 == 0) yun = 1;
    var arr = [31, 28 + yun, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31];
    for (var i = 0; i < month - 1; i++) day += arr[i];
    ...
    for (var i = 0; i < 6; i++) {
      for (var j = 0; j < 7; j++) {
        if (i == 0 && j < firstDay)
          ...
          if (j == 0) calendar[i][j] = new DayData(month, ++date, "RED", "");
          else if (j == 6)
            calendar[i][j] = new DayData(month, ++date, "BLUE", "");
          else calendar[i][j] = new DayData(month, ++date, "BLACK", "");
        }
      }
    }
  ...
  function DayObj(m_day) {
    ...
    this.set_day_number = function (value) {
      ...
      value =
        value.length == 2
          ? value
          : new Array(2 - value.length + 1).join("0") + value;
  ...
  function make_days(days, Day) {
    //만든 day들을 담을 배열, 복사할 원본 day
    let today_date = new Date();

    let calendar = CreateAMonthlyCalendar(
    ...
    for (var i = 0; i < 6; i++) {
      for (var j = 0; j < 5; j++) {
        ...
        days[i][j].set_color(calendar[i][j + 1].color);

        let month = today_date.getMonth() + days[i][j].get_month_offset();
        //console.log(month);
        day_callback(
          days[i][j],
          new Date(today_date.getFullYear(), month, days[i][j].get_day_number())
        );
      }
    }
  }
...
function set_today_text() {
  ...
  document.querySelector(".Day_Of_The_WeeK").innerHTML =
    week[today_date.getDay()];
}
