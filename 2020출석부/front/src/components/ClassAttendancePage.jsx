...
class ClassAttendancePage extends PureComponent {
  ...
  async componentWillMount() {
    ...
    try {
      const res = await Axios.get(`http://13.209.68.218/class/${floor}`, { headers: { accessToken } });
      const { classes } = res.data;
      await this.setState({ classes, selectedClass: classes[0].id });
      const { selectedClass, period } = this.state;
      const res2 = await Axios.get(`http://13.209.68.218/attendance/class/${selectedClass}?period=${period}`, { headers: { accessToken } });
  ...
  onClick = (id) => async () => {
    ...
    const accessToken = localStorage.getItem('accessToken');
    try {
      const res = await Axios.get(`http://13.209.68.218/attendance/class/${id}?period=${period}`, { headers: { accessToken } });
  ...
  onClickPeriod = (period) => async () => {
    ...
    try {
      const accessToken = localStorage.getItem('accessToken');
      const res = await Axios.get(`http://13.209.68.218/attendance/class/${selectedClass}?period=${period}`, { headers: { accessToken } });
      const { attendances } = res.data;
  ...
  render() {
    const { selectedClass, classes, period, attendanceList, unsaved } = this.state;
    const date = new Date();
    ...
    const classesList = classes.map((v, i) => <li onClick={this.onClick(v.id)} className={v.id === selectedClass ? 'club-li-now' : ''}><div className={v.id === selectedClass ? 'club-a-now' : ''}>{v.grade}학년 {v.class}반</div></li>)
    return (
      <section>
        <div id="class-select-bar">
          <ul>
            <li><input onClick={date.getDay() !== 5 ? () => alert('오늘은 7교시가 없습니다.') : this.onClickPeriod(7)} type="button" value="7교시" id="class-7" className={date.getDay() !== 5 ? 'before-class' : period === 7 ? 'now-class' : ''} /></li>
