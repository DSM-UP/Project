...
class StudentStats extends PureComponent {
  ...
  onChangeGrade = async (e) => {
    ...
    try {
      const res = await Axios.get(
        `http://3.34.125.239/student?grade=${gradeValue}&classs=${classValue}`,
        { headers: { accessToken } }
      );
      const { students } = res.data;
      const res2 = await Axios.get(
        `http://3.34.125.239/attendance/student/${students[0].id}?page=0`,
        { headers: { accessToken } }
      );
  ...
  onChangeClass = async (e) => {
    ...
    try {
      const res = await Axios.get(
        `http://3.34.125.239/student?grade=${gradeValue}&classs=${e.target.value}`,
        { headers: { accessToken } }
      );
      const { students } = res.data;
      const res2 = await Axios.get(
  ...
  onClickDown = (next) => async () => {
    ...
    try {
      const res = await Axios.get(
        `http://3.34.125.239/attendance/student/${studentId}?page=${page + 1}`,
        { headers: { accessToken } }
      );
  ...
  clickStudent = async (id) => {
    ...
    try {
      const res = await Axios.get(
        `http://3.34.125.239/attendance/student/${id}?page=0`,
        { headers: { accessToken } }
      );
      const { attendances } = res.data;
  ...
  async componentWillMount() {
    ...
    try {
      ...
      const { students } = res.data;
      const res2 = await Axios.get(
        `http://3.34.125.239/attendance/student/${studentId}?page=${page}`,
        { headers: { accessToken } }
      );
      const { attendances } = res2.data;
  ...
  render() {
    ...
    const studentList = students.map((v, i) => (
      <StudentList
    ...
    return (
      <section>
        <div id="section-wrap">
          ...
          <div id="section-student-space">{studentList}</div>
        </div>
