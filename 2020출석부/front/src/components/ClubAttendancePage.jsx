...
class ClubAttendancePage extends PureComponent {
  ...
  async componentWillMount() {
    ...
    try {
      const res = await Axios.get(`http://3.34.125.239/club/${floor}`, {
        headers: { accessToken },
      });
      ...
      const res2 = await Axios.get(
        `http://3.34.125.239/attendance/club/${selectedClub}?period=${period}`,
        { headers: { accessToken } }
      );
  ...
  onClick = (id) => () => {
    ...
    const accessToken = localStorage.getItem("accessToken");
    Axios.get(`http://3.34.125.239/attendance/club/${id}?period=${period}`, {
      headers: { accessToken },
    })
      .then((res) => {
            ...
            {clubs.map((v, i) => {
              return (
                <li
                  ...
                  className={v.id === selectedClub ? "club-li-now" : ""}
                >
                  <div
                    className={`${v.id === selectedClub ? "club-a-now" : ""}`}
                  >
                    {v.name}
                  </div>
                </li>
