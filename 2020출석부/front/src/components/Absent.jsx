...
class Abesent extends PureComponent {
  ...
  componentWillMount() {
    ...
    Axios.get(`http://3.34.125.239/student?grade=1&classs=1`, {
      headers: { accessToken },
    })
      .then((res) => {
        this.setState({
          students: res.data.students,
          selectedStudent: `${res.data.students[0].id} ${res.data.students[0].name}`,
        });
      })
      .catch((err) => {
  ...
  onChangeStartClass = (e) => {
    this.setState({ startClass: e.target.value });
  };
  onChangeEndClass = (e) => {
    this.setState({ endClass: e.target.value });
  };
  onChangeGrade = (e) => {
    ...
    const accessToken = localStorage.getItem("accessToken");
    Axios.get(
      `http://3.34.125.239/student?grade=${e.target.value}&classs=${selectedClass}`,
      { headers: { accessToken } }
    )
      .then((res) => {
        this.setState({
  ...
  onChangeClass = (e) => {
    ...
    const accessToken = localStorage.getItem("accessToken");
    Axios.get(
      `http://3.34.125.239/student?grade=${selectedGrade}&classs=${e.target.value}`,
      { headers: { accessToken } }
    )
      .then((res) => {
        this.setState({
  ...
  onSubmit = (e) => {
    ...
    const accessToken = localStorage.getItem("accessToken");
    Axios.post(
      `http://3.34.125.239/absence/${studentId}`,
      { reason, purpose, from, to },
      { headers: { accessToken } }
    )
      .then((res) => {
      ...
      <>
        <section>
          <div id="start-date">
            ...
            />
            <select
              onChange={this.onChangeStartClass}
              value={startClass}
              name=""
              id=""
            >
          ...
          <div id="end-date">
            ...
            />
            <select
              onChange={this.onChangeEndClass}
              value={endClass}
              name=""
              id=""
            >
          ...
          <form id="absent-form" onSubmit={this.onSubmit}>
            <div id="absent-form-purpose">
              ...
              <input
                className={selectedPurpose === "현체" ? "select" : null}
              ...
              />
              <input
                className={selectedPurpose === "외출" ? "select" : null}
                onClick={this.onClick}
                type="button"
                value="외출"
                id="outing-button"
              />
              <input
                className={selectedPurpose === "공결" ? "select" : null}
                onClick={this.onClick}
                type="button"
                value="공결"
                id="absent-button"
            ...
            <label
              htmlFor="absent-form-purpose-reason"
              id="absent-form-reason-label"
            >
              비고(사유)
            </label>
            <input
